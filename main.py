...
kurucu_id = 6532412571

admins = [kurucu_id, 6532412571]
...
"""
    ...
    if konumlar != False:
        if await is_admin(chat_id, user_id):
            ...
            if oyun_tipi == "sessizsinema", or  oyun_tipi == "sessiz_sinema":
                skorlar = f(f"games.{oyun_id}.skorlar")
                #round = int(f(f"games.{oyun_id}.round")) + 1
                #toplam_round = f(f"games.{oyun_id}.toplam_round")
...
"""
                for n, i in enumerate(skorlar_list):
                    ...
                    metin += "\n"
                keyboard = types.InlineKeyboardMarkup()
                callback_button = types.InlineKeyboardButton(text="Tekrar oyna 🔃", callback_data="sessiz_sinema")
                keyboard.add(callback_button)
                await bot.send_message(chat_id, metin, reply_markup=keyboard)
            else:
                keyboard = types.InlineKeyboardMarkup()
